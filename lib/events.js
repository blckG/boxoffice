...
let TorrentClient = new Deluge(settings.value().torrentClientAddress || 'http://192.168.1.100:8112', 'deluge');
...
emitter.on('system:recon', () => {
  // XXX: recreates out deluge client connection
  // used for reconnecting after changing client address from settings
  TorrentClient = new Deluge(settings.value().torrentClientAddress || 'http://192.168.1.100:8112', 'deluge');
});
...
function getMovie(movie, done) {
  ...
  yify.movieList({query_term: movie.imdbId}).then(json => {
    if (json.status === 'ok') {
...
schedule.scheduleJob('*/5 * * * * *', () => {
  TorrentClient.getActiveTorrents()
    .then(client => {
      ...
      socket.emit('toaster:toast', `torrent client error: ${err.code}`, 'error');
      console.log(err);
    });
});
